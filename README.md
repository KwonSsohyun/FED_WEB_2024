...
| `10 week` | Day 22 | 24.09.12 | • 상태 변경 메서드 적용(@action) <br>• 상태 변경 시 자동 재계산 및 값 캐싱 get 함수(computed) <br>• 비동기 작업 중 상태 변경(runInAction) <br>• MobX 이벤트 시스템 <br>• Observable 값 변경 시 자동 실행(autorun) <br>• 상태 값이 바뀔 때만 콜백 함수 실행(reaction) <br>• 조건 만족 시 단일 실행(when) |
